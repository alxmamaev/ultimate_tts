...
class Tacotron2(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            vocab_size + 1,  # One extra token for paddinng
    ...
    def forward(self, texts, encoder_mask, mels_target):
        ...
        (
            ...
            rnn_states,
            alignment_state,
        ) = self.decoder.get_inital_states(memory)
        ...
        postnet_outputs = (
            self.postnet(decoder_outputs.transpose(1, 2)).transpose(1, 2)
            + decoder_outputs
        )
    ...
    def inference(
        ...
        **kwargs
        ...
        if mels_target is None:
            postnet_outputs, alignments, decoder_mask = self._inference(
                texts, encoder_mask, gate_th, max_decoder_steps
            )
        else:
            # For ground truth alignment
            decoder_outputs, postnet_outputs, alignments, decoder_mask = self.forward(
                texts, encoder_mask, mels_target
            )
